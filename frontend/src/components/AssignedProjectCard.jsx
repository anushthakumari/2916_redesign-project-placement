...
export const AssignedProjectCard=({data,projectName,dateJoined,supservisorName,status,assingmentId,completedPercentage,problemStatement})=> {
  ...
  return (
    <Card sx={{ minWidth: 275 ,width:600,mt:3,cursor:"pointer"}}>
      <CardContent >
        ...
        <Stack m={'.5rem 0rem'} >
          <Typography gutterBottom variant="body2">
              {isExpanded?(problemStatement):(problemStatement?.substring(0, 200))}
              {problemStatement}
          </Typography>
        </Stack>
      ...
      <CardActions>
        <Button size="medium" variant='contained'>{completedPercentage===100?"Completed":"Open"}</Button>
      </CardActions>
    </Card>
  );
}